...
const getImageUrl = (specificImage, type = 'words', keyword = '') => {
  ...
  if (defaultImages[searchTerm]) return defaultImages[searchTerm];
  const defaultImage = type === 'phrases' ? defaultImages['phrases'] : defaultImages['words'];
  return defaultImage;
};
...
function App() {
  ...
  const renderTypingExercise = () => {
    ...
    const { currentWord, userInput, targetKeys, currentKeyIndex, keyboardHighlight, wordCompleted, errorMessage } = typingExercise;
          ...
          <div className="card-body">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1 flex flex-col gap-2">
                {(isShiftPressed ? ['shiftRow1', 'shiftRow2', 'shiftRow3', 'shiftRow4'] : ['row1', 'row2', 'row3', 'row4']).map((rowName) => {
                  return (
                    <div key={rowName} className="flex justify-center gap-1">
                      {thaiKeyboard[rowName].map((key, index) => {
                        const isHighlighted = keyboardHighlight &&
                          ((isShiftPressed && keyboardHighlight.shift && keyboardHighlight.row.replace('shift', '') === rowName.replace('shift', '') && keyboardHighlight.index === index) ||
  ...
  const renderMemoryExercise = () => {
    ...
    const { targetWord, userInput, currentWordIndex, words, showHint, keyboardHighlight, completed } = memoryExercise;
